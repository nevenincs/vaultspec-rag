...
- [ ] `S03` - Replace the citations in the indexer, search, and server modules on the same basis; `src/vaultspec_rag/indexer/__init__.py, src/vaultspec_rag/search/__init__.py, src/vaultspec_rag/server/_lifecycle.py, src/vaultspec_rag/server/_lifespan.py, src/vaultspec_rag/server/_main.py, src/vaultspec_rag/server/_models.py`.
- [ ] `S04` - Replace the citations in the test modules, keeping synthetic vault paths that are legitimate test data; `src/vaultspec_rag/tests`.
- [ ] `S06` - Remove development-record citations from the mcp package docstrings and comments, keeping any protocol-tool names that are runtime contract rather than record references; `src/vaultspec_rag/mcp`.
...
- [ ] `S05` - Guard the boundary with a check that fails when a tracked source file names a development record, so the invariant is enforced rather than asserted; `src/vaultspec_rag/tests/test_adr_regression.py`.
## Description
