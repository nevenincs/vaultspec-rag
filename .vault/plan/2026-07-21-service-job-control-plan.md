...
- [x] `W03.P11.S22` - Verify real daemon restart and shutdown preserve queued and paused intent, mark interrupted attempts, and close stores only after worker release using vaultspec-high-executor; `src/vaultspec_rag/tests/integration/test_service_lifecycle.py`.
...
- [x] `W04.P12.S26` - Verify authenticated real-ASGI job CRUD, exact mutations, revisions, idempotency, capacity, force rejection, retry linkage, deletion conflicts, and Location headers using vaultspec-standard-executor; `src/vaultspec_rag/tests/integration/test_service_jobs.py`.
