...
- [ ] `W01.P02.S14` - Verify full, scoped, API, CLI, and service admission parity against one real temporary repository; `src/vaultspec_rag/tests/integration/test_content_admission.py`.
...
- [ ] `W01.P02.S92` - Route ordinary added and modified watcher paths through the shared disposition and active policy snapshot; `src/vaultspec_rag/watcher.py`.
- [ ] `W01.P02.S93` - Verify watcher classification matches full and scoped discovery for ordinary real file events; `src/vaultspec_rag/tests/integration/test_content_admission.py`.
