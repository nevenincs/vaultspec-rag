...
- [ ] `P03.S16` - apply the existing flush-cadence throttle to the vault slice path, which currently empties the CUDA cache every slice; `src/vaultspec_rag/indexer/_streaming.py`.
- [ ] `P03.S17` - throttle the document per-file loop's cache release, which currently syncs the device every slice by defaulting release-cache on; `src/vaultspec_rag/indexer/_document_indexer.py`; `src/vaultspec_rag/indexer/_streaming.py`.
