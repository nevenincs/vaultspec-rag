...
- [ ] `P01.S01` - Route the embed-format and config-drift escalation gates to failure-safe reconciliation so no watcher-driven run reaches a destructive rebuild; `src/vaultspec_rag/indexer/_codebase_indexer.py`.
...
- [ ] `P01.S04` - Prove by real-storage test that an unattended incremental over a drifted config leaves the served point count intact, and that the same run under an explicit operator rebuild still drops the collection; `src/vaultspec_rag/tests/integration/test_index_job_control.py`.
