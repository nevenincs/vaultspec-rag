...
- [ ] `W04.P08.S31` - Bind preflight compatibility and authenticated service-state observation to one ready discovered identity, require finite safe-quiesce transition timestamps, fail closed with stable typed refusal outcomes, and prove the boundary through real routes without GPU authority or local fallback; `src/vaultspec_rag/cli/_service_preflight.py, src/vaultspec_rag/tests/test_service_preflight_cli.py`.
