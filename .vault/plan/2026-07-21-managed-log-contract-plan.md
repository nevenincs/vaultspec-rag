...
- [ ] `W01.P02.S04` - Implement bounded raw-byte rotation and configure the Qdrant supervisor from the shared retention policy; `src/vaultspec_rag/qdrant_runtime/_supervise.py`.
...
- [ ] `W02.P03.S06` - Replace the legacy service-only reader with bounded source-aware grouped log retrieval; `src/vaultspec_rag/logging_config.py`.
...
- [ ] `W02.P05.S12` - Replace the legacy activity parser and raw compatibility flag with grouped source rendering and offline fallback; `src/vaultspec_rag/cli/_service_logs.py`.
