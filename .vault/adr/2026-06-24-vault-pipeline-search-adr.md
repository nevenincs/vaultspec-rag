...
date: '2026-06-24'
modified: '2026-07-27'
body_hash: 'sha256:a63247dd00a6ccbf02c433ce4e9ce982cf77798167b6167ddf979615e1236501'
related:
...
0.4642 â€” retrieved, but out-ranked by its own artifact by a 0.38 margin. The results are
...
- The vault search pipeline (`src/vaultspec_rag/search/_searcher.py`) runs hybrid RRF â†’
  CrossEncoder rerank (calibrated sigmoid [0,1]) â†’ chunk grouping â†’ graph rerank. Any
...
- An in-tree precedent exists: `_apply_prefer_nudge` reweights by category post-rerank and
  ...
  requires deliberately overriding relevance â€” the central tension this ADR resolves.
...
- Ranking/intent/status logic must live in the search/service domain with the CLI as a
  thin adapter (`service-domain-owns-operability`); every parameter threads CLI â†’ HTTP
  route â†’ MCP `search_vault`, with a stable JSON envelope.
...
**D1 â€” Intent is an explicit mode, never inferred.** Search takes a declared intent. Two
profiles ship first â€” `orientation` (default) and `debug` â€” selected by an explicit
...
**D2 â€” A multiplicative per-(type Ã— status) ranking prior.** After the CrossEncoder
...
still win across types â€” while deliberately allowing a high-value type to overcome a
relevance gap. This is the intentional, bounded override of the "tie-break only"
philosophy: it is scoped to the typeÃ—status axis, gated on explicit intent, and inspectable.
The graph in-link/feature nudges remain bounded tie-breakers applied within the reweighted
ordering.

**D3 â€” Weights live in config, are legible and tunable.** The orientation and debug
...
orientation starting profile: adr/audit â‰ˆ 1.0, research/reference â‰ˆ 0.85, plan â‰ˆ 0.6, exec
â‰ˆ 0.4; debug inverts exec/summary to the top. Status multipliers (orientation): accepted
and unknown â‰ˆ 1.0, proposed â‰ˆ 0.6, superseded/rejected/deprecated â‰ˆ 0.3 â€” subject to
tuning against D8.

**D4 â€” Per-type cap to fight crowding.** A bounded cap on how many results of one doc_type
...
**D5 â€” Status extraction, vocabulary, and the `--status` control.** The indexer extracts
...
**D6 â€” Doc-type union as first-class selection, back-compatible.** `--type vault` remains
...
**D7 â€” Frontmatter-enriched results.** `SearchResult` gains `related` and `status`. The
...
**D8 â€” The intent-aware validation instrument (shared acceptance gate).** A three-tier,
all-real-inference instrument, because the existing surfaces cannot grade role Ã— intent:
...
Tier 1 â€” *offline graded metrics*. A `quality`-marked integration test drives a committed
labeled query set (~30â€“50 real queries, ~10â€“15 per intent) against a real index. Each query
declares its intent and a gold list of `(doc_id, grade)` judgments on a 0â€“3 scale assigned
*mechanically* from a declarative rubric keyed on (intent, doc_type, status) â€” authored
...
**role-precision@3** sanity guard; and the existing **needle-precision floor (â‰¥0.75)** as a
hard no-regress guard. The live failure ("decision on gpu lock scope" â†’ the
...
Tier 2 â€” *qualitative persona testimonials*. Extend the existing scripted-persona discipline
...
signal) â€” the same data viewed two ways.

Tier 3 â€” *A/B delta report*: a regenerable before/after table of rank-1 doc and score per
gold query, embedded in the audit as the decision evidence.

Corpus: enrich the synthetic generator to emit status markers and pipeline (researchâ†’adrâ†’
planâ†’exec) edges for a deterministic CI gate, with periodic runs against the real `.vault/`
for confidence. A precondition the instrument exposes: `status` must be surfaced onto
`SearchResult` and the payload (D5/D7) before any role Ã— status metric can be computed.

**D9 â€” Search-quality tooling is test-suite tooling, not a production verb.** The
...
(collapsing plan+exec under the governing ADR via `related`) â€” both valuable, both
sequenced after the scoring prior proves out.### Implementation erratum (2026-06-24)
...
- **Rule slug:** `vault-ranking-prior-is-intent-scoped`.
  **Rule:** The vault doc-type/status ranking prior may override semantic relevance only
  under an explicitly declared intent mode, only on the typeÃ—status axis, and must remain
