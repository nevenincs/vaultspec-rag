...
- Measured validation (byte-near-identical fork of this repository, live service, code domain, single shared 16 GB GPU): the flag-OFF from-scratch rebuild baseline is 311 s (~1,988 code chunks, ~4.6-5.0 GB peak CUDA, constrained support profile and encode batch to fit the shared GPU). The anticipated drop to chunk + lookup + upsert time was NOT realised: with the flag ON against a present, eligible byte-identical sibling donor, the run still performed a full GPU encode (~311 s, ~4.6 GB peak CUDA) with zero adopted vectors and no reuse telemetry - an effective 0% hit rate. Donor discovery and eligibility were independently verified correct in isolation (the sibling donor is selected and passes every gate with the fork's real content-epoch, which is byte-identical between the two roots), and the off-switch resolves enabled in a daemon-identical environment. The gap is therefore a blocking defect in the live service full-rebuild path - donor reuse does not engage there - not a donor-selection or eligibility miss. The upsert-plus-preallocation floor could not be isolated below the full-encode cost because no run adopted vectors. The headline speedup and the floor remain uncaptured pending resolution of the live-path defect and a re-run.
