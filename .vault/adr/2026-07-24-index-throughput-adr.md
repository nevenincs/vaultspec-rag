...
STATUS OF CLAIMS: Part A is now MEASURED on the running system; Part B is MEASURED and confirms it buys correctness rather than speed; the flush-cadence flip stays UNMEASURED and untaken, and the gRPC transport switch stays untaken. The numbers below carry their status inline. The measurement that mattered did not need a staged window: the resident daemon has run the gated build for hours across four watched roots, and its own persisted job records are the contended window, decomposed per job into admission wait, run window, internal work timer, and in-run GPU-lock wait.
