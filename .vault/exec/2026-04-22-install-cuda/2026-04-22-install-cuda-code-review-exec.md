...
date: 2026-04-22
modified: '2026-07-27'
body_hash: 'sha256:a492b7381fbecb6eda6be53c7bbbe71fb7f9382b04fd830996ad424b34789a3c'
related:
...
**`2026-04-12-vaultspec-rag-install-adr`** â€” install/uninstall
layering. Verdict: **adhered**.

- Orchestration stays in `commands.py`; Typer wrappers in `cli.py`
  remain thin (args â†’ call â†’ render â†’ exit). New flags
...
- No direct mutation of `.gitignore`, `.gitattributes`,
  `.mcp.json`, or provider dirs â€” only the user's `pyproject.toml`,
  which the ADR established as user-owned (consent-gated).

**`2026-04-06-ecosystem-integration-adr`** â€” companion
...
- `diagnose_torch` is tested as a pure function over (cuda,
  available) args â€” no torch stubbing required.
...
- `NO_TORCH` â€” `ImportError` branch; tested indirectly via the
  error-handler smoke run.
- `CPU_ONLY` â€” `torch.version.cuda is None`; tested in
  `test_diagnose_torch[None-False-cpu_only]` and the anomaly case
  `[None-True-cpu_only]`.
- `NO_GPU` â€” CUDA str present, `is_available()` False; tested.
- `WORKING` â€” passthrough; tested.
- Bare `cli.py:1652` check migrated to call `_handle_gpu_error`
  with a `RuntimeError("CUDA runtime unavailable")` â€” verified by
...
- Canonical-match predicate (`_index_match`, `_source_match`)
  used on both apply and remove paths â€” single source of truth.
...
- Uninstall now runs torch-config removal even when `.vaultspec/`
  is absent â€” discovered during testing; fix landed before commit.
...
- Second call returns `action="already"` without touching the file
  (SHA-256 compare).
- Third call, and any subsequent, same outcome â€” the canonical-match
