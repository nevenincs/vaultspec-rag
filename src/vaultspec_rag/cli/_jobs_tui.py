...
class ServerWatchApp(App[None]):
    ...
    def on_mount(self) -> None:
        ...
        self.screen.set_class(self._watch_mode == "jobs", "-jobsfocused")
        self.set_interval(_SPINNER_INTERVAL, self._tick)
        self.set_interval(self._interval, self.refresh_jobs)
        self.set_interval(self._interval, self.refresh_search_activity)
        self.set_interval(self._interval, self.refresh_managed_logs)
    ...
    def _apply_search_activity(
        ...
        self._search_counts = {
            name: _count(counts.get(name)) or 0
...
def _search_activity_payload_error(result: dict[str, object]) -> str | None:
    ...
    if _count(returned) is None:
        return "served-search activity unavailable: invalid returned count"
    for name in ("active", "recent", "total"):
        if _count(cast("dict[str, object]", counts).get(name)) is None:
