...
class TestCommitGateDoesNotDeadlock:
    """The spec-check commit hook gates on hard failures, never on warnings.

    `spec doctor` exits 1 on warnings, and warning-level provider-mirror lag is
    the expected steady state here because a separate gate forbids committing
    regenerated mirrors. A hook that treats exit 1 as a failure therefore blocks
    every commit after any builtins change, and the observed response is that
    contributors start passing --no-verify - which disables every other hook in
    the file as collateral.

    The hardened wrapper has been reverted before by a tool that regenerated the
    config, and nothing noticed until commits started failing. This binds it.
    """

    def _hook_entry(self) -> str:
        from pathlib import Path

        config = Path(__file__).resolve().parents[3] / ".pre-commit-config.yaml"
        assert config.is_file(), f"pre-commit config not found at {config}"
        text = config.read_text(encoding="utf-8")
        marker = "- id: spec-check"
        assert marker in text, "the spec-check hook is missing from the commit gate"
        after = text.split(marker, 1)[1]
        entry_line = next(
            (line for line in after.splitlines() if line.strip().startswith("entry:")),
            "",
        )
        assert entry_line, "the spec-check hook has no entry line"
        return entry_line

    def test_spec_check_gates_on_hard_failures_only(self):
        entry = self._hook_entry()
        # The exit-code comparison is the whole guard: a bare `spec doctor`
        # entry propagates its warning-level exit 1 and deadlocks the commit.
        assert "returncode < 2" in entry, (
            "the spec-check hook must gate on exit code >= 2 (hard failures). "
            "A bare `vaultspec-core spec doctor` entry exits 1 on warnings, "
            "which are expected here, and deadlocks every commit. Found: "
            f"{entry.strip()}"
        )
