...
def test_a_never_indexed_root_answers_rather_than_failing(
    ...
    """The first search on a new project must not be an error.

    A root nobody has indexed has no publication proof, and every read that
    reaches for one has to treat that as an absence rather than a refusal.
    ...
    The verdict is asserted, not only the status. Returning 200 while
    reporting nothing about breadth would hide a strict read behind a
    swallowed exception; `unverifiable` is the typed outcome that says the
    proof could not be read and no claim was invented in its place.
