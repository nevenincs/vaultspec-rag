...
def test_search_freshness_wait_max_default_and_settings_projection() -> None:
    previous = os.environ.pop(
        EnvVar.SEARCH_FRESHNESS_WAIT_MAX_SECONDS.value,
        None,
    )
    try:
        reset_config()
...
def test_search_freshness_wait_max_environment_and_explicit_override() -> None:
    # Removing the explicit override precedence returned the environment's 12.5;
    # the exact 18.25 assertion failed. Restoration passed this test.
    ...
    try:
        ...
        assert (
            get_config(
                {"search_freshness_wait_max_seconds": 18.25}
...
def test_search_freshness_wait_max_accepts_closed_boundaries(value: int) -> None:
    assert get_config(
        {"search_freshness_wait_max_seconds": value}
    ).search_freshness_wait_max_seconds == float(value)
    reset_config()
...
def test_search_freshness_wait_max_rejects_invalid_explicit_values(
    ...
    # Weakening either closed range edge admitted -0.01 or 300.01 and failed
    # with DID NOT RAISE; admitting bool/nonfinite did likewise, while admitting
    # a string exposed float's raw conversion error and failed the exact schema
    # message assertion. Each narrow mutation was restored and this test passed.
    expected = (
...
def test_search_freshness_wait_max_rejects_malformed_environment_exactly() -> None:
    # Returning the default from the environment parse-error branch produced
    # DID NOT RAISE; restoration passed this exact source-aware error assertion.
...
def test_search_freshness_wait_max_rejects_out_of_range_environment_exactly(
    ...
    # Bypassing post-parse bound admission let env -0.01 resolve and produced
    # DID NOT RAISE; restoration passed this exact source-aware assertion.
