...
def test_a_non_terminal_transition_is_never_reported_as_a_failure(
    ...
    events = _log(state, caplog, tmp_path / "root")
...
def test_a_failure_event_always_carries_a_populated_error(
    ...
    """
    _log(state, caplog, tmp_path / "root")
    failed_lines = [
        record.getMessage()
        for record in caplog.records
...
def test_supersession_is_its_own_outcome_at_ordinary_severity(
    ...
    """
    events = _log(JobState.SUPERSEDED, caplog, tmp_path / "root")
...
def test_every_job_state_has_an_explicit_outcome(
    ...
    reported_failed = {
        ...
        if any(
            name == "reindex_failed"
            for name, _ in _log(state, caplog, tmp_path / state.value)
        )
    }
