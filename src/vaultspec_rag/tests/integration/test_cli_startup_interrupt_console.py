...
``CTRL_C_EVENT``. ``threading.interrupt_main`` cannot stand in. It trips the
interpreter's flag directly and skips the console handler the CRT runs on its
own thread, so it exercises none of the delivery path that produced the
failure - a guard built on it reports success against a machine that never
receives the signal.

Delivery has two Windows properties that the arrangement below is built
around, both of which silently produce a passing test if got wrong:

- ``CTRL_C_EVENT`` reaches processes by CONSOLE, not by pid, so the child is
  given its own console and the event is generated for everyone on it;
- the "ignore Ctrl+C" attribute is INHERITED by child processes. A test
  process that set it - to survive the event it generates - would hand every
  CLI it spawns a deafness indistinguishable from a working guard. So the
  generating process is a separate short-lived one that spawns nothing and is
  free to make itself deaf.
...
if sys.platform == "win32":
    ...
    def _clear_inherited_console_interrupt_deafness() -> None:
        ...
        started by a launcher that set it - shells and task runners commonly
        do. A CLI spawned under an inherited ignore runs straight through the
        interrupt, so the guard would be measured against a process that never
        received the signal and would pass no matter what the entry point did.
        Clearing it restores the ordinary console default rather than imposing
        anything, and it cannot expose this process to the event below, which
        is generated only on the child's own console.
        """
        kernel32 = ctypes.WinDLL("kernel32", use_last_error=True)
    ...
    def _spawn_cli_holding_the_window_open(marker: Path) -> subprocess.Popen[str]:
        ...
        child created without it can never receive the event. It is hidden
        because a test suite should not flash windows, and both streams stay on
        pipes so the console never sees the output.
        """
