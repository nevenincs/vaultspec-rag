...
def test_running_phase_status_failure_rolls_back_all_started_components(
    ...
    with _service_env(tmp_path, env_overrides=offline_env):
        ...
        model_deadline = time.monotonic() + 120.0
        daemon_log = tmp_path / "service.log"
        output = ""
        while time.monotonic() < model_deadline and _is_pid_alive(daemon_pid):
...
def test_startup_expiry_reaps_pre_readiness_qdrant(
    ...
    with _service_env(tmp_path, env_overrides=offline_env):
        ...
        assert status_before_parent["port"] == port
        assert status_before_parent["phase"] == "warming"
        assert status_before_parent["qdrant_version"] == "1.18.2"
        assert float(status_before_parent["qdrant_start_time"]) > 0.0
        nested_identity = cast(
            "dict[str, object]",
            status_before_parent["qdrant_identity"],
        )
        assert nested_identity["pid"] == qdrant_pid
        assert nested_identity["port"] == qdrant_port
        assert nested_identity["version"] == "1.18.2"
        assert (
            nested_identity["start_time"] == status_before_parent["qdrant_start_time"]
        )
        ...
        assert _wait_for_exit(pid, timeout=30.0), (
            f"startup-expired service launcher {pid} did not exit"
        )
    ...
    def test_posix_attached_qdrant_publishes_existing_child_identity(
        ...
        with _service_env(tmp_path):
            ...
            try:
                ...
                assert (
                    after_heartbeat["qdrant_start_time"]
                    == status["qdrant_start_time"]
                )
            finally:
