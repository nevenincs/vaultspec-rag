...
Like the ceiling above it, the floor derives by default rather than shipping a fixed figure: at `0` it comes from the CUDA demand the configured support profile declares, so it scales with the workload and the card instead of asserting one machine's measurements. A positive value overrides the derivation for your own device.
